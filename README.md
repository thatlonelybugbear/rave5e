...
- Requires Foundry VTT 14.365 or newer and supports dnd5e 5.3.3.
- Evaluation uses the affected actor and, where applicable, the current item or activity being rolled. It does not inspect targets or roll dialog choices.
...
Use item keys such as `activities[<activity-id>].damage.parts`, `system.damage.types`, `name`, or `img` when you want the owned item itself to temporarily change. This can work like a conditional enchantment without permanently editing the item.
...
| `movementLastSegment` | Combat movement cost for the actor token's most recent movement segment, when an active combat and token are available. |
| `movementTurn` | Accumulated combat movement cost for the actor token, when an active combat and token are available. |
